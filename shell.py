...
"""Interactive shell and shell-side command runner for NovaDev 1.1."""
...
SHELL_HELP = """NovaDev shell commands:
  ...
  .ast <file> [--json]
  .ir <file-or-project>
  .explain <file-or-project>
  .validate <file-or-project>
  ...
  .build-mode <file> [-o generated]
  .new <project-name>
  .lint <file-or-project>
  .format <file>
  .docs <file-or-project>
  .clean <file-or-project>
  .dev <file-or-project>
...
def execute_command(command: str, args: List[str], interpreter: Optional[Interpreter] = None) -> int:
    ...
    if command == "tokens":
        path = one_file_arg(args)
        for token in tokenize(read_source(source_entry(path))):
    ...
    if command in {"new", "lint", "format", "docs", "clean", "dev", "install", "update", "remove", "package", "deploy", "ir", "explain", "validate", "build-mode"}:
        import nova

        return nova.main([command, *args])
...
def run_buffer(buffer: List[str], interpreter: Interpreter) -> None:
    ...
    if not source:
        return
    try:
        runtime = interpreter.run(source)
        if runtime.last_value is not None:
            print(runtime.format_value(runtime.last_value))
...
def interactive_main() -> int:
    ...
    print(f"NovaDev {__version__} Interactive Shell")
