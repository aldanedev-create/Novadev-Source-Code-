...
"""NovaDev 1.1 command-line interface."""
...
from novadev.project_ir_builder import ProjectIRBuilder
...
def source_entry(path: Path) -> Path:
    if path.is_dir():
        config = path / "Nova.toml"
        if config.exists():
...
def command_tokens(args) -> int:
    for token in tokenize(read_source(source_entry(args.file))):
...
def command_ir(args) -> int:
    program = parse_file(args.file)
    ir = ProjectIRBuilder().build(program)
    print(json.dumps(ir.to_data(), indent=2))
    return 0
...
def command_explain(args) -> int:
    program = parse_file(args.file)
    ir = ProjectIRBuilder().build(program)
    print(f"NovaDev {__version__} project explanation")
    print(f"App: {ir.name}")
    print(f"Mode: {ir.mode}")
    print(f"Styling: {ir.styling} ({ir.style.mode} profile)")
    for note in ir.notes:
        print(f"- {note}")
    print(f"- Entities: {', '.join(entity.name for entity in ir.entities) or 'none'}")
    print(f"- Pages: {', '.join(f'{page.name}:{page.type}' for page in ir.pages) or 'none'}")
    print(f"- Workflows: {', '.join(workflow.name for workflow in ir.workflows) or 'none'}")
    print(f"- Modules: {', '.join(f'{module.name}:{module.language}' for module in ir.modules) or 'none'}")
    if ir.mode == "custom":
        print("- mode custom: no unrelated domain defaults are generated.")
    return 0
...
def command_validate(args) -> int:
    ...
    print(f"NovaDev validate ok: {args.file}")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="nova", description="NovaDev 1.1 language CLI")
    ...
    new_parser.add_argument("--styling", default="Tailwind")
    ...
    ir_parser = subparsers.add_parser("ir", help="Print NovaDev 1.1 ProjectIR JSON")
    ir_parser.add_argument("file", type=Path)
    ir_parser.set_defaults(func=command_ir)

    explain_parser = subparsers.add_parser("explain", help="Explain project-specific generation decisions")
    explain_parser.add_argument("file", type=Path)
    explain_parser.set_defaults(func=command_explain)

    validate_parser = subparsers.add_parser("validate", help="Validate ProjectIR, workflows, modules, and mode rules")
    ...
    build_mode_parser = subparsers.add_parser("build-mode", help="Build using NovaDev 1.1 mode-specific generation")
    build_mode_parser.add_argument("file", type=Path)
    build_mode_parser.add_argument("-o", "--output", type=Path, default=Path("generated"))
    build_mode_parser.set_defaults(func=command_build)
