...
class Runtime:
    ...
    def execute(self, node: Any) -> Any:
        ...
        if isinstance(node, ClassNode):
            klass = NovaClass(node.name, {method.name: method for method in node.methods})
    ...
    def call_method(self, target: Any, method: str, args: List[Any]) -> Any:
        ...
        if isinstance(target, NovaInstance):
            if method not in target.klass.methods:
                raise NovaRuntimeError(f"Class '{target.klass.name}' has no method '{method}'")
            return self.call_user_function(target.klass.methods[method], args, self_value=target)
    ...
    def callee_name(self, node: Any) -> str:
        if isinstance(node, IdentifierNode):
            return node.name
        raise NovaRuntimeError("Can only call named functions in NovaDev 0.4")
    ...
    def interpolate(self, text: str) -> str:
        ...
        def replace(match):
            ...
            try:
                return self.format_value(self.resolve_name(name))
            except Exception:
                return "{" + name + "}"
    ...
    def assign_target(self, name: str, value: Any) -> None:
        parts = name.split(".")
        if len(parts) == 1:
