...
"""Interactive shell and shell-side command runner for NovaDev 0.4."""
...
def parse_backend_args(args: List[str]) -> Tuple[Path, Path, Path]:
    file_path: Optional[Path] = None
    ...
    if file_path is None:
        raise ValueError("Expected a .nova file path")
    return file_path, output_path, frontend_path
...
def execute_command(command: str, args: List[str], interpreter: Optional[Interpreter] = None) -> int:
    ...
    if command in {"load", "run"}:
        ...
        active_interpreter.run(read_source(path))
        return 0
    ...
    if command == "build-backend":
        path, output, frontend = parse_backend_args(args)
        generated = BackendGenerator().generate(parse_file(path), output, frontend)
        print(f"NovaDev {__version__} backend generated in {output}")
        for item in generated:
            print(f"  {item}")
        return 0
...
def interactive_main() -> int:
    ...
    print("NovaDev 0.4 Interactive Shell")
