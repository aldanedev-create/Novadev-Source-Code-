...
.nova source
  ...
  -> AST declarations
  -> runtime registries
  -> UI generator / Flask backend generator
  -> generated files
```
...
between prompts. It also mirrors the `nova.py` command surface with `.run`,
`.tokens`, `.ast`, `.build-ui`, `.build-backend`, `.version`, and `.help`.
...
The generated Flask app serves `dist/index.html`, `dist/style.css`, and
`dist/app.js`, then exposes table APIs under `/api/...`.
